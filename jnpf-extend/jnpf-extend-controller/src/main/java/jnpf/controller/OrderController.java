...
@Slf4j
@Tag(name = "订单信息", description = "Order")
@RestController
@RequestMapping("/api/extend/CrmOrder")
@RequiredArgsConstructor
public class OrderController extends SuperController<OrderService, OrderEntity> {
     ...
     * 获取订单信息列表（带分页）
     *
     * @param paginationOrder 分页模型
     * @return
     */
    @Operation(summary = "获取订单信息列表")
    @GetMapping
    @SaCheckPermission("extend.order")
    public ActionResult<PageListVO<OrderListVO>> list(PaginationOrder paginationOrder) {
        ...
        List<String> id = data.stream().map(OrderEntity::getId).collect(Collectors.toList());
        List<TaskEntity> orderStaList = workFlowApi.getInfosSubmit(id.toArray(new String[]{}), TaskEntity::getStatus, TaskEntity::getId, TaskEntity::getFlowId);
        List<String> userId = listVO.stream().map(OrderListVO::getCreatorUserId).collect(Collectors.toList());
        ...
        for (OrderListVO order : listVO) {
            ...
            order.setCreatorUser(user != null ? user.getRealName() + "/" + user.getAccount() : "");
            TaskEntity task = orderStaList.stream().filter(t -> t.getId().equals(order.getId())).findFirst().orElse(null);
            order.setCurrentState(task != null ? task.getStatus() : 0);
            order.setFlowId(task != null ? task.getFlowId() : order.getFlowId());
        }
    ...
    @SaCheckPermission("extend.order")
    public ActionResult<ListVO<OrderEntryListVO>> orderEntryList(@PathVariable("id") String id) {
        ...
        ListVO<OrderEntryListVO> vo = new ListVO<>();
        vo.setList(result);
    ...
    @Parameter(name = "id", description = "主键", required = true)
    @SaCheckPermission("extend.order")
    public ActionResult<ListVO<OrderReceivableListVO>> orderReceivableList(@PathVariable("id") String id) {
        ...
        ListVO<OrderReceivableListVO> vo = new ListVO<>();
        vo.setList(result);
        return ActionResult.success(vo);
    }
    ...
    @Parameter(name = "id", description = "主键", required = true)
    @SaCheckPermission("extend.order")
    public ActionResult<OrderInfoVO> info(@PathVariable("id") String id) {
        ...
        OrderInfoVO infoVo = JsonUtil.getJsonToBean(orderEntity, OrderInfoVO.class);
        if (infoVo != null) {
            ...
            if (StringUtils.isNotEmpty(infoVo.getCreatorUserId())) {
                UserEntity creator = userService.getInfo(infoVo.getCreatorUserId());
                infoVo.setCreatorUserId(creator != null ? creator.getRealName() + "/" + creator.getAccount() : "");
            }
            if (StringUtils.isNotEmpty(infoVo.getLastModifyUserId())) {
                UserEntity lastModify = userService.getInfo(infoVo.getLastModifyUserId());
                infoVo.setLastModifyUserId(lastModify != null ? lastModify.getRealName() + "/" + lastModify.getAccount() : "");
            }
        }
    ...
    @Parameter(name = "id", description = "主键", required = true)
    @SaCheckPermission("extend.order")
    public ActionResult<OrderInfoVO> prevInfo(@PathVariable("id") String id) throws DataException {
    ...
    @Parameter(name = "id", description = "主键", required = true)
    @SaCheckPermission("extend.order")
    public ActionResult<OrderInfoVO> nextInfo(@PathVariable("id") String id) throws DataException {
    ...
    @Parameter(name = "id", description = "主键", required = true)
    @SaCheckPermission("extend.order")
    public ActionResult<DownloadVO> exportExcel(@PathVariable("id") String id) {
        OrderEntity orderEntity = orderService.getInfo(id);
        if (orderEntity != null) {
            if (StringUtil.isNotEmpty(orderEntity.getCreatorUserId())) {
                UserEntity user = userService.getInfo(orderEntity.getCreatorUserId());
                orderEntity.setCreatorUserId(user != null ? user.getRealName() + "/" + user.getAccount() : "");
            }
            if (StringUtil.isNotEmpty(orderEntity.getLastModifyUserId())) {
                UserEntity user = userService.getInfo(orderEntity.getLastModifyUserId());
                orderEntity.setLastModifyUserId(user != null ? user.getRealName() + "/" + user.getAccount() : "");
            }
            OrderExportModel exportModel = JsonUtil.getJsonToBean(orderEntity, OrderExportModel.class);
            //保存数据
            Map<String, Object> map = new HashMap<>(16);
            map.put("order", JSON.parse(JSON.toJSONStringWithDateFormat(exportModel, "yyyy-MM-dd HH:mm:ss")));
            List<OrderEntryEntity> orderEntryList = orderService.getOrderEntryList(id);
            String num = "0";
            ...
            try {
                //模板
                TemplateExportParams param = new TemplateExportParams(configValueUtil.getTemplateFilePath() + "orderInfo_export_template.xlsx", true);
                @Cleanup Workbook workbook = ExcelExportUtil.exportExcel(param, map);
                ...
                vo.setName(name);
                vo.setUrl(UploaderUtil.uploaderFile(name));
            } catch (Exception e) {
                log.error("信息导出Excel错误:{}", e.getMessage());
            }
            return ActionResult.success(vo);
        }
        return ActionResult.fail(MsgCode.ETD104.get());
    }
     ...
     *
     * @param id        主键
     * @param orderForm 订单模型
     * @return
     */
    @Operation(summary = "新建订单信息")
    @PostMapping("/{id}")
    @Parameter(name = "id", description = "主键", required = true)
    @Parameter(name = "orderForm", description = "订单模型", required = true)
    @SaCheckPermission("extend.order")
    public ActionResult<Object> create(@PathVariable("id") String id, @RequestBody @Valid OrderForm orderForm)  {
        ...
        orderService.create(orderEntity, orderEntryList, orderReceivableList);
    ...
    @Parameter(name = "orderForm", description = "订单模型", required = true)
    @SaCheckPermission("extend.order")
    public ActionResult<Object> update(@PathVariable("id") String id, @RequestBody @Valid OrderForm orderForm)  {
        OrderEntity orderEntity = JsonUtil.getJsonToBean(orderForm, OrderEntity.class);
        List<OrderEntryEntity> orderEntryList = JsonUtil.getJsonToList(orderForm.getGoodsList(), OrderEntryEntity.class);
        List<OrderReceivableEntity> orderReceivableList = JsonUtil.getJsonToList(orderForm.getCollectionPlanList(), OrderReceivableEntity.class);
        ...
        orderEntity.setId(id);
        if (entity != null) {
            orderService.update(id, orderEntity, orderEntryList, orderReceivableList);
    ...
    @GetMapping("/Customer")
    @SaCheckPermission("extend.order")
    public ActionResult<ListVO<OrderGoodsVO>> customerList(Page page) {
        ...
        ListVO<OrderGoodsVO> vo = new ListVO<>();
        vo.setList(list);
        return ActionResult.success(vo);
    }

...
public class DbLinkController extends SuperController<DbLinkService, DbLinkEntity> {
    ...
    public ActionResult<ListVO<DbLinkSelectorListVO>> selectorList(String type) {
        ...
        if(type == null){
            ...
            dbLink.setId("0");
            dbLink.setDbType(TenantDataSourceUtil.getDataSourceConfig().getDbType());
    ...
    public ActionResult<PageListVO<DbLinkListVO>> getList(PaginationDbLink page) {
        ...
        List<String> userId = data.stream().map(DbLinkEntity::getCreatorUserId).collect(Collectors.toList());
        List<String> lastUserId = data.stream().map(DbLinkEntity::getLastModifyUserId).collect(Collectors.toList());
    ...
    @SaCheckPermission("dataCenter.dataSource")
   ...
   @Parameter(name = "dbLinkCreUpForm", description = "新建数据连接表单对象", required = true)
    ...
    public ActionResult<String> create(@RequestBody @Valid DbLinkCreUpForm dbLinkCreUpForm) {
        ...
        if (dblinkService.isExistByFullName(entity.getFullName(), null)) {
    ...
    @SaCheckPermission("dataCenter.dataSource")
    public ActionResult<String> test(@RequestBody DbLinkBaseForm dbLinkBaseForm) {

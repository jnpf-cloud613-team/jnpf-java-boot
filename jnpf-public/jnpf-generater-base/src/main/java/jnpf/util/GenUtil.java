...
public class GenUtil {
    ...
    public QueryWrapper solveValue(QueryWrapper<?> wrapper, String fieldDb) {
        ...
        try {
            ArrayList<String> splitKey = new ArrayList<>();
            splitKey.add(JnpfKeyConsts.DATE);
            splitKey.add(JnpfKeyConsts.DATE_CALCULATE);
            splitKey.add(JnpfKeyConsts.TIME);
            splitKey.add(JnpfKeyConsts.NUM_INPUT);
            splitKey.add(JnpfKeyConsts.CREATETIME);
            splitKey.add(JnpfKeyConsts.MODIFYTIME);
            ...
            selectIgnore = new ArrayList<>();
            selectIgnore.add(JnpfKeyConsts.ADDRESS);
            selectIgnore.add(JnpfKeyConsts.CASCADER);
            selectIgnore.add(JnpfKeyConsts.CHECKBOX);
        ...
        void judge(QueryWrapper<?> wrapper, String fieldDb) {
            ...
            switch (operator) {
                ...
                case NOT_NULL:
                    ...
                    break;
                case "==":
                    if ((JnpfKeyConsts.DATE.equals(jnpfKey) || JnpfKeyConsts.DATE_CALCULATE.equals(jnpfKey)) && null != time) {
    ...
    public List<String> solveListValue(String fieldValue) {
        ArrayList<String> result = new ArrayList<>();
        try {
            List<List> list = JsonUtil.getJsonToList(fieldValue, List.class);
            for (List listSub : list) {
                result.add(JSON.toJSONString(listSub));
                // 组织选择需要取最后每个数组最后一个
            ...
            result.add(JSON.toJSONString(list));

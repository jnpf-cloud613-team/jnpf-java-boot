...
import jnpf.database.model.query.ConditionJsonModel;
import jnpf.database.model.query.SuperJsonModel;
import jnpf.database.model.query.SuperQueryConditionModel;
...
public class GeneraterSwapUtil {
    ...
    public static final String PRO_MAP = "__pro_map";
    public static final String SSD = "%s-%s-%d";
    public static final String NOT_LIKE = "notLike";
    public static final String ERRORS_INFO = "errorsInfo";
    ...
    private final OnlineSwapDataUtils swapDataUtils;
    ...
    public String provinceData(String data, Map<String, Object> localCache) {
        ...
        if (localCache != null && localCache.containsKey(PRO_MAP)) {
            proMap = (Map<String, String>) localCache.get(PRO_MAP);
        }
    ...
    public Map<String, Object> localCache() {
        ...
        //省市区
        Map<Object, Object> proMap = redisUtil.getMap(String.format(SSD, dsName, PROVINCE, 1));
        List<Map<String, String>> proMapList = new ArrayList<>();
        if (CollectionUtils.isNotEmpty(proMap)) {
            //分级存储
            for (int i = 1; i <= 4; i++) {
                String redisKey = String.format(SSD, dsName, PROVINCE, i);
            ...
            for (int i = 1; i <= 4; i++) {
                proMapList.add(redisUtil.getMap(String.format(SSD, dsName, PROVINCE, i)));
            }
        }

        Map<String, String> proMapr = new HashMap<>();
        proMapList.forEach(proMapr::putAll);
        localCache.put(PRO_MAP, proMapr);
        return localCache;
    }
    ...
    public String comSelectValues(String ids, Boolean mul) {
        ...
        String separator = Boolean.TRUE.equals(mul) ? "," : "/";
        if (ids.contains("[[")) {
    ...
    public String swapRelationFormValue(String vmodel, String value, String modelId, Map<String, Object> formDataMaps) {
        ...
        try {
            ...
            VisualdevModelDataInfoVO infoVO = visualDevInfoService.getDetailsDataInfo(value, entity);
            if (infoVO != null) {
                Map<String, Object> formDataMap = infoVO.getData() != null ? JsonUtil.stringToMap(infoVO.getData()) : new HashMap<>();
                if (CollectionUtils.isNotEmpty(formDataMaps)) {
    ...
    public String getPopupSelectValue(String interfaceId, String propsValue, String relationField, String dataValue, Map<String, Object> dataMaps) {
        ...
        if (StringUtil.isNotEmpty(dataValue)) {
            ...
            if (data instanceof ActionResult) {
                ActionResult<Object> actionVo = (ActionResult<Object>) data;
    ...
    public String getDicName(String feild, String dictionaryTypeId) {
        if (StringUtil.isNotEmpty(feild)) {
            ...
            if (feilds.length > 1) {
                ...
                String finalValue;
                if (StringUtils.isEmpty(feildsValue)) {
    ...
    public String getDynName(String urlId, String label, String value, String feildValue, String json, int num, Map<String, Object> dataAll) {
        List<TemplateJsonModel> list = JsonUtil.getJsonToList(json, TemplateJsonModel.class);
        Map<String, String> infoMap = !list.isEmpty() ? new HashMap<>() : null;
        for (TemplateJsonModel templateJsonModel : list) {
            if (StringUtil.isNotEmpty(templateJsonModel.getRelationField())) {
                String[] mastTable = templateJsonModel.getRelationField().split(JnpfConst.SIDE_MARK);
                String[] child = templateJsonModel.getRelationField().split("-");
        ...
        if (StringUtil.isNotEmpty(feildValue)) {
            //去除中括号以及双引号
            feildValue = feildValue.replaceAll(REG_EX, "");
            //获取远端数据
            ActionResult<Object> object = dataInterfaceService.infoToId(urlId, null, infoMap);
            if (object.getData() != null && object.getData() instanceof DataInterfaceActionVo) {
                ...
                if (feildValues.length > 0) {
                    ...
                    String finalValue;
                    if (StringUtils.isEmpty(feildVa)) {
    ...
    public String getDynName(String urlId, String name, String id, String children, String feildValue, boolean mul) {
        ...
        if (StringUtil.isNotEmpty(feildValue)) {
            ...
            ActionResult<Object> object = dataInterfaceService.infoToId(urlId, null, null);
    ...
    public String getDynName(String urlId, String name, String id, String children, String feildValue, boolean mul, boolean isFullPath, String json, int num, Map<String, Object> dataAll1) {
        ...
        if (StringUtil.isNotEmpty(feildValue)) {
            ...
            ActionResult<Object> data = dataInterfaceService.infoToId(urlId, null, infoMap);
    ...
    public Integer getCondition(SuperQueryConditionModel<?> conditionModel, Object entity, int num) {
        ...
        for (ConditionJsonModel queryConditionModel : useCondition) {
            ...
            if (StringUtil.isEmpty(fieldValue)) {
                ...
                } else if (queryConditionModel.getSymbol().equals(NOT_LIKE)) {
                    queryConditionModel.setSymbol("<>");
                }
            }
                ...
                if ("AND".equalsIgnoreCase(op)) {
                    if (symbol.equals("==")) {
                        tw.and(qw -> {
                                    ...
                                    multJnpf.add(JnpfKeyConsts.CASCADER);
                                    multJnpf.add(JnpfKeyConsts.COMSELECT);
                                    multJnpf.add(JnpfKeyConsts.ADDRESS);
                                    multJnpf.add(JnpfKeyConsts.SELECT);
                                    multJnpf.add(JnpfKeyConsts.TREESELECT);
                    ...
                    } else if (symbol.equals(NOT_LIKE)) {
                        tw.and(ew -> {
                            if (StringUtil.isNotEmpty(fieldValue)) {
    ...
    public static Map<String, Object> getMastTabelData(Object data, String tableName) {
        ...
        for (Map.Entry<String, Object> entry : map.entrySet()) {
            ...
            String[] jnpfS = key.split(JnpfConst.SIDE_MARK);
            if (jnpfS.length == 2 && jnpfS[0].contains(tableName)) {
                mapRes.put(jnpfS[1], map.get(key));
            }
        }
        return mapRes;
    }
     ...
     */
    @DS("")
    public void getFlowStatus(List<Map<String, Object>> realList) {
        try {
            swapDataUtils.getFlowStatus(realList);
        } catch (Exception e) {
            log.error("流程状态异常：" + e.getMessage(), e);
    ...
    public static List<Map<String, Object>> groupTable(List<Map<String, Object>> realList, String groupField, String firstField) {
        ...
        for (Map.Entry<String, List<Map<String, Object>>> entry : twoMap.entrySet()) {
            String key = entry.getKey();
    ...
    public Map<String, Object> swapDataForm(Map<String, Object> dataMap, GenerParamConst paramConst) {
        ...
        //数据转换
        return this.swapDataForm(dataMap, fields, paramConst.getTableFieldKey(), paramConst.getTableRenames());
    }

    private Map<String, Object> swapDataForm(Map<String, Object> dataMap, List<FieLdsModel> fields
        ...
        for (FieLdsModel item : fields) {
            ...
            Boolean isMultiple = Objects.nonNull(item.getMultiple()) && item.getMultiple();

            //获取原字段数据
            FormPublicUtils.relationGetJnpfId(dataMap, jnpfKey, dataMap.get(vModel), vModel);

            List<String> systemConditions = new ArrayList<>();
            systemConditions.add(JnpfKeyConsts.CURRORGANIZE);
            systemConditions.add(JnpfKeyConsts.CURRDEPT);
            systemConditions.add(JnpfKeyConsts.CURRPOSITION);
            //多选二维数组
            List<String> multTow = new ArrayList<>();
            multTow.add(JnpfKeyConsts.CASCADER);
            multTow.add(JnpfKeyConsts.ADDRESS);
            //一维维数组
            List<String> multOne = new ArrayList<>();
            multOne.add(JnpfKeyConsts.CHECKBOX);
            List<String> nullIsList = new ArrayList<>();
            nullIsList.add(JnpfKeyConsts.UPLOADFZ);
            nullIsList.add(JnpfKeyConsts.UPLOADIMG);
            if (Objects.nonNull(dataMap.get(vModel))) {
                if (multTow.contains(jnpfKey) && Boolean.TRUE.equals(isMultiple)) {
                    //二维数据转换
                    dataMap.replace(vModel, JSON.parseArray(dataMap.get(vModel).toString(), List.class));
                } else if (Boolean.TRUE.equals(multTow.contains(jnpfKey) || isMultiple) || multOne.contains(jnpfKey)) {
                    //一维数据转换
                    dataMap.replace(vModel, JSON.parseArray(dataMap.get(vModel).toString(), String.class));
                }
            ...
            switch (jnpfKey) {
                ...
                case JnpfKeyConsts.MODIFYUSER:
                    UserEntity userEntity = userService.getInfo(String.valueOf(dataMap.get(vModel)));
                    String s = userEntity.getAccount().equalsIgnoreCase(ADMIN_KEY)
                            ? "管理员/" + ADMIN_KEY : userEntity.getRealName() + "/" + userEntity.getAccount();
                    String userValue = s;
                ...
                case JnpfKeyConsts.CHILD_TABLE:
                    ...
                    for (int i = 0; i < childList.size(); i++) {
                        ...
                        childList.set(i, this.swapDataForm(childMap, childrens, tableField, tableRename));
                    }
    ...
    public ExcelImportModel importData(GenerParamConst paramConst, List<Map<String, Object>> dataList, ImportFormCheckUniqueModel uniqueModel) throws WorkFlowException {
        ...
        DbLinkEntity linkEntity = linkService.getInfo(uniqueModel.getDbLinkId());
        ...
        if (StringUtil.isNotEmpty(uniqueModel.getFlowId())) {
            List<TemplateJsonEntity> flowVersionIds = workFlowApi.getFlowIdsByTemplate(uniqueModel.getFlowId());
            ...
            for (int i = 0, len = dataList.size(); i < len; i++) {
                ...
                if (uniqueModel.isUpdate()) {
                    ...
                    if (ObjectUtil.isNotEmpty(visualErrInfo) && StringUtil.isNotEmpty(visualErrInfo.getErrMsg())) {
                        errorMap.put(ERRORS_INFO, visualErrInfo.getErrMsg());
                        failResult.add(errorMap);
                        continue;
                    }
                    String excelHas = formCheckUtils.checkBusinessKeyExcel(formDataModel, resultMap, uniqueModel);
                    if (StringUtil.isNotEmpty(excelHas)) {
                        errorMap.put(ERRORS_INFO, excelHas);
                        failResult.add(errorMap);
                        continue;
                    }
                }
                ...
                if (errInfo.length() > 0) {
                    errorMap.put(ERRORS_INFO, errInfo.toString());
                    failResult.add(errorMap);
                } else {
        ...
        importModel.setResultType(!failResult.isEmpty() ? 1 : 0);
    ...
    public static String swapDatetime(JSONArray fieldsStr, Object obj, Map<String, String> tableRename) {
        ...
        for (Map.Entry<String, String> entry : tableRename.entrySet()) {
            String tabelRealName = entry.getKey();
    ...
    public MPJLambdaWrapper getConditionAllTable(QueryAllModel queryAllModel) {
        ...
        if (queryAllModel.getModuleId() != null && Boolean.FALSE.equals(userInfo.getIsAdministrator())) {
        ...
        Map<String, String> systemFieldValue = userApi.getSystemFieldValue(new SystemParamModel(json));
     ...
     */
    @DS("")
    public PageListVO<Map<String, Object>> getInterfaceData(Object pagination, GenerViewConst paramConst, Boolean hasPage, String viewKey) {
        ...
        if (isPc && columnDataModel.getType() == 3) {
            //分组数据转换
            interfaceData = this.groupTable(interfaceData, paramConst.getGroupField(), paramConst.getFirstField());
        }
        //返回对象
    ...
    public DownloadVO creatModelExcel(List<ExcelExportEntity> entitys, List<Map<String, Object>> list, String[] keys, String menuId, String moduleId,
                                      GenerParamConst generParamConst, boolean inlineEdit) throws IOException {
        //数据转换
    ...
    public List<String> getFlowIds(String templateId) {
        ...
        if (StringUtil.isNotEmpty(templateId)) {
            flowIds.addAll(workFlowApi.getFlowIdsByTemplateId(templateId));
            if (CollectionUtils.isNotEmpty(flowIds)) {
                return flowIds;
            }
            String templateByVersionId = workFlowApi.getTemplateByVersionId(templateId);
            if (StringUtil.isNotEmpty(templateByVersionId)) {
                flowIds.addAll(workFlowApi.getFlowIdsByTemplateId(templateByVersionId));
            }
        }
    ...
    public String deleteFlowTask(String flowTaskId) {
        String errMsg = "";
        TaskEntity taskEntity = workFlowApi.getInfoSubmit(flowTaskId, TaskEntity::getId, TaskEntity::getParentId, TaskEntity::getFullName, TaskEntity::getStatus);
        if (taskEntity != null) {
            try {

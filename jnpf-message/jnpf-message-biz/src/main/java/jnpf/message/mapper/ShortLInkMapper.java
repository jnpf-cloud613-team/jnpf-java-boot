...
public interface ShortLInkMapper extends SuperMapper<ShortLinkEntity> {
    ...
    default ShortLinkEntity getInfoByLink(String link) {
    ...
    default String shortLink(String link) {
     ...
     * @return
     */
    static String md5ByHex(String src) {
        try {
            ...
            for (int i = 0; i < hash.length; i++) {
                String stmp = Integer.toHexString(hash[i] & 0xFF);
                if (stmp.length() == 1) {
                    hs.append('0');
                }
                hs.append(stmp);
